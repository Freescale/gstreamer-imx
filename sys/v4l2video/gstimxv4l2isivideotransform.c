...
GstImxV4L2ISIVideoTransformQueue;
...
struct _GstImxV4L2ISIVideoTransform
{
	...
	GstImxV4L2ISIVideoTransformQueue v4l2_output_queue, v4l2_capture_queue;
};


struct _GstImxV4L2ISIVideoTransformClass
{
...
G_DEFINE_TYPE(GstImxV4L2ISIVideoTransform, gst_imx_v4l2_isi_video_transform, GST_TYPE_BASE_TRANSFORM)

/* General element operations. */
static void gst_imx_v4l2_isi_video_transform_dispose(GObject *object);
static void gst_imx_v4l2_isi_video_transform_set_property(GObject *object, guint prop_id, GValue const *value, GParamSpec *pspec);
static void gst_imx_v4l2_isi_video_transform_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);
static GstStateChangeReturn gst_imx_v4l2_isi_video_transform_change_state(GstElement *element, GstStateChange transition);

/* Caps handling. */
static GstCaps* gst_imx_v4l2_isi_video_transform_transform_caps(GstBaseTransform *transform, GstPadDirection direction, GstCaps *input_caps, GstCaps *filter);
static GstCaps* gst_imx_v4l2_isi_video_transform_fixate_caps(GstBaseTransform *transform, GstPadDirection direction, GstCaps *caps, GstCaps *othercaps);
static GstCaps* gst_imx_v4l2_isi_video_transform_fixate_size_caps(GstBaseTransform *transform, GstPadDirection direction, GstCaps *caps, GstCaps *othercaps);
static void gst_imx_v4l2_isi_video_transform_fixate_format_caps(GstBaseTransform *transform, GstCaps *caps, GstCaps *othercaps);
static gboolean gst_imx_v4l2_isi_video_transform_set_caps(GstBaseTransform *transform, GstCaps *input_caps, GstCaps *output_caps);

/* Allocator. */
static gboolean gst_imx_v4l2_isi_video_transform_decide_allocation(GstBaseTransform *transform, GstQuery *query);

/* Frame output. */
static GstFlowReturn gst_imx_v4l2_isi_video_transform_prepare_output_buffer(GstBaseTransform *transform, GstBuffer *input_buffer, GstBuffer **output_buffer);
static GstFlowReturn gst_imx_v4l2_isi_video_transform_transform_frame(GstBaseTransform *transform, GstBuffer *input_buffer, GstBuffer *output_buffer);
static gboolean gst_imx_v4l2_isi_video_transform_transform_size(GstBaseTransform *transform, GstPadDirection direction, GstCaps *caps, gsize size, GstCaps *othercaps, gsize *othersize);

/* Metadata and meta information. */
static gboolean gst_imx_v4l2_isi_video_transform_transform_meta(GstBaseTransform *trans, GstBuffer *input_buffer, GstMeta *meta, GstBuffer *output_buffer);
static gboolean gst_imx_v4l2_isi_video_transform_copy_metadata(GstBaseTransform *trans, GstBuffer *input_buffer, GstBuffer *output_buffer);

/* GstImxV4L2ISIVideoTransform specific functions. */
...
static gboolean gst_imx_v4l2_isi_video_transform_enable_stream(GstImxV4L2ISIVideoTransform *self, GstImxV4L2ISIVideoTransformQueue *queue, gboolean do_enable);


static void gst_imx_v4l2_isi_video_transform_class_init(GstImxV4L2ISIVideoTransformClass *klass)
{
	GObjectClass *object_class;
	GstElementClass *element_class;
	GstBaseTransformClass *base_transform_class;

	GST_DEBUG_CATEGORY_INIT(imx_v4l2_isi_video_transform_debug, "imxv4l2isivideotransform", 0, "NXP i.MX V4L2 ISI video transform element");
	...
	object_class->dispose      = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_dispose);
	object_class->set_property = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_set_property);
	object_class->get_property = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_get_property);

	element_class->change_state = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_change_state);

	base_transform_class->transform_caps        = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_transform_caps);
	base_transform_class->fixate_caps           = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_fixate_caps);
	base_transform_class->set_caps              = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_set_caps);
	base_transform_class->decide_allocation     = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_decide_allocation);
	base_transform_class->transform             = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_transform_frame);
	base_transform_class->transform_size        = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_transform_size);
	base_transform_class->prepare_output_buffer = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_prepare_output_buffer);
	base_transform_class->transform_meta        = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_transform_meta);
	base_transform_class->copy_metadata         = GST_DEBUG_FUNCPTR(gst_imx_v4l2_isi_video_transform_copy_metadata);
	...
	gst_element_class_set_static_metadata(
		element_class,
		"i.MX V4L2 ISI video transform",
		"Filter/Converter/Video/Scaler/Transform/Effect/Hardware",
		"Video transformation using the i.MX8 ISI (Image Sensing Interface) through the V4L2 mem2mem API",
		"Carlos Rafael Giani <crg7475@mailbox.org>"
	);
}


void gst_imx_v4l2_isi_video_transform_init(GstImxV4L2ISIVideoTransform *self)
{
...
static void gst_imx_v4l2_isi_video_transform_dispose(GObject *object)
{
	GstImxV4L2ISIVideoTransform *self = GST_IMX_V4L2_ISI_VIDEO_TRANSFORM(object);

	g_free(self->device);
	gst_caps_replace(&(self->v4l2_output_queue.available_caps), NULL);
	gst_caps_replace(&(self->v4l2_capture_queue.available_caps), NULL);

	G_OBJECT_CLASS(gst_imx_v4l2_isi_video_transform_parent_class)->dispose(object);
}
...
static GstStateChangeReturn gst_imx_v4l2_isi_video_transform_change_state(GstElement *element, GstStateChange transition)
{
	GstImxV4L2ISIVideoTransform *self = GST_IMX_V4L2_ISI_VIDEO_TRANSFORM(element);
	...
	switch (transition)
	{
		...
		{
			if (!gst_imx_v4l2_isi_video_transform_open(self))
	...
	ret = GST_ELEMENT_CLASS(gst_imx_v4l2_isi_video_transform_parent_class)->change_state(element, transition);
	...
	switch (transition)
	{
		case GST_STATE_CHANGE_READY_TO_NULL:
			gst_imx_v4l2_isi_video_transform_close(self);
			break;
...
static GstCaps* gst_imx_v4l2_isi_video_transform_transform_caps(GstBaseTransform *transform, GstPadDirection direction, GstCaps *input_caps, GstCaps *filter)
{
	GstImxV4L2ISIVideoTransform *self = GST_IMX_V4L2_ISI_VIDEO_TRANSFORM(transform);
	GstCaps *stripped_input_caps, *unfiltered_caps, *transformed_caps;
	GstImxV4L2ISIVideoTransformQueue *v4l2_transform_queue;
 ...
 * gst_imx_v4l2_isi_video_transform_fixate_caps
 * gst_imx_v4l2_isi_video_transform_fixate_size_caps
 * score_value
 * gst_imx_v4l2_isi_video_transform_fixate_format_caps
 * */
...
static GstCaps* gst_imx_v4l2_isi_video_transform_fixate_caps(GstBaseTransform *transform, GstPadDirection direction, GstCaps *caps, GstCaps *othercaps)
{
	...
	result = gst_imx_v4l2_isi_video_transform_fixate_size_caps(transform, direction, caps, result);
	GST_DEBUG_OBJECT(transform, "fixated size to: %" GST_PTR_FORMAT, (gpointer)result);

	gst_imx_v4l2_isi_video_transform_fixate_format_caps(transform, caps, result);
...
static GstCaps* gst_imx_v4l2_isi_video_transform_fixate_size_caps(GstBaseTransform *transform, GstPadDirection direction, GstCaps *caps, GstCaps *othercaps)
{
	...
	gst_imx_v4l2_isi_video_transform_fixate_format_caps(transform, caps, othercaps);
...
static gboolean gst_imx_v4l2_isi_video_transform_set_caps(GstBaseTransform *transform, GstCaps *input_caps, GstCaps *output_caps)
{
	...
	gst_imx_v4l2_isi_video_transform_teardown_v4l2_queue(self, &(self->v4l2_output_queue));
	gst_imx_v4l2_isi_video_transform_teardown_v4l2_queue(self, &(self->v4l2_capture_queue));
	...
	if (!gst_imx_v4l2_isi_video_transform_setup_v4l2_queue(self, &(self->v4l2_output_queue), &video_info))
		return FALSE;
	...
	if (!gst_imx_v4l2_isi_video_transform_setup_v4l2_queue(self, &(self->v4l2_capture_queue), &video_info))
		return FALSE;
...
static gboolean gst_imx_v4l2_isi_video_transform_decide_allocation(GstBaseTransform *transform, GstQuery *query)
{
	...
	return GST_BASE_TRANSFORM_CLASS(gst_imx_v4l2_isi_video_transform_parent_class)->decide_allocation(transform, query);
}
	...
	*output_buffer = NULL;
	...
	if (!(self->v4l2_capture_queue.stream_enabled))
	{
		for (i = 0; i < self->v4l2_capture_queue.num_buffers; ++i)
		{
			...
			ret = gst_imx_v4l2_isi_video_transform_queue_buffer(self, &(self->v4l2_capture_queue), gstbuffer);
		...
		if (!gst_imx_v4l2_isi_video_transform_enable_stream(self, &(self->v4l2_capture_queue), TRUE))
			goto error;
	}
	else
	{
	...
	if (self->v4l2_output_queue.stream_enabled)
	{
		...
		previous_input_buffer = gst_imx_v4l2_isi_video_transform_dequeue_buffer(self, &(self->v4l2_output_queue));
		gst_buffer_unref(previous_input_buffer);
	}

	GST_LOG_OBJECT(self, "queuing new V4L2 output buffer to process upstream frame");
	if (!gst_imx_v4l2_isi_video_transform_queue_buffer(self, &(self->v4l2_output_queue), input_buffer))
		goto error;

	if (!(self->v4l2_output_queue.stream_enabled))
	{
		if (!gst_imx_v4l2_isi_video_transform_enable_stream(self, &(self->v4l2_output_queue), TRUE))
			goto error;		
	}

	GST_LOG_OBJECT(self, "dequeuing V4L2 capture buffer to retrieve converted frame");
	*output_buffer = gst_imx_v4l2_isi_video_transform_dequeue_buffer(self, &(self->v4l2_capture_queue));
	if (G_UNLIKELY(*output_buffer == NULL))
		goto error;

	gst_imx_v4l2_isi_video_transform_copy_metadata(transform, original_input_buffer, *output_buffer);
...
static GstFlowReturn gst_imx_v4l2_isi_video_transform_transform_frame(G_GNUC_UNUSED GstBaseTransform *transform, G_GNUC_UNUSED GstBuffer *input_buffer, G_GNUC_UNUSED GstBuffer *output_buffer)
{
...
static gboolean gst_imx_v4l2_isi_video_transform_transform_size(G_GNUC_UNUSED GstBaseTransform *transform, G_GNUC_UNUSED GstPadDirection direction, G_GNUC_UNUSED GstCaps *caps, G_GNUC_UNUSED gsize size, GstCaps *othercaps, gsize *othersize)
{
	GstVideoInfo video_info;
...
static gboolean gst_imx_v4l2_isi_video_transform_transform_meta(GstBaseTransform *trans, GstBuffer *input_buffer, GstMeta *meta, GstBuffer *output_buffer)
{
	...
	return GST_BASE_TRANSFORM_CLASS(gst_imx_v4l2_isi_video_transform_parent_class)->transform_meta(trans, input_buffer, meta, output_buffer);
}


static gboolean gst_imx_v4l2_isi_video_transform_copy_metadata(G_GNUC_UNUSED GstBaseTransform *trans, GstBuffer *input_buffer, GstBuffer *output_buffer)
{
...
static gboolean gst_imx_v4l2_isi_video_transform_open(GstImxV4L2ISIVideoTransform *self)
{
	...
	{
		GST_ERROR_OBJECT(self, "creating DMA-BUF buffer allocator failed");
		goto error;
	}
	...
	if (!gst_imx_v4l2_isi_video_transform_probe_available_caps(self, &(self->v4l2_capture_queue)))
	{
...
static void gst_imx_v4l2_isi_video_transform_teardown_v4l2_queue(GstImxV4L2ISIVideoTransform *self, GstImxV4L2ISIVideoTransformQueue *queue)
{
	...
	gst_imx_v4l2_isi_video_transform_enable_stream(self, queue, FALSE);
