...
GST_DEBUG_CATEGORY (fsl_ipubufferpool_debug);
...
static void gst_fsl_ipu_buffer_pool_finalize(GObject *object);


G_DEFINE_TYPE(GstFslIpuBufferPool, gst_fsl_ipu_buffer_pool, GST_TYPE_BUFFER_POOL)




static const gchar ** gst_fsl_ipu_buffer_pool_get_options(G_GNUC_UNUSED GstBufferPool *pool)
{
...
static gboolean gst_fsl_ipu_buffer_pool_set_config(GstBufferPool *pool, GstStructure *config)
{
	...
	fsl_ipu_pool = GST_FSL_IPU_BUFFER_POOL(pool);
...
static GstFlowReturn gst_fsl_ipu_buffer_pool_alloc_buffer(GstBufferPool *pool, GstBuffer **buffer, G_GNUC_UNUSED GstBufferPoolAcquireParams *params)
{
	GstFslIpuBufferPool *fsl_ipu_pool;
	GstBuffer *buf;
	GstMemory *mem;
	...
	alloc_params.flags = fsl_ipu_pool->read_only ? GST_MEMORY_FLAG_READONLY : 0;
	alloc_params.align = 0;

	info = &fsl_ipu_pool->video_info;
	...
	mem = gst_allocator_alloc(fsl_ipu_pool->allocator, info->size, &alloc_params);
	if (mem == NULL)
	{
...
static void gst_fsl_ipu_buffer_pool_finalize(GObject *object)
{
	GstFslIpuBufferPool *fsl_ipu_pool = GST_FSL_IPU_BUFFER_POOL(object);

	GST_DEBUG_OBJECT(fsl_ipu_pool, "shutting down IPU buffer pool");
	
	G_OBJECT_CLASS (gst_fsl_ipu_buffer_pool_parent_class)->finalize(object);
	 ...
	 * exist */
	gst_object_unref(fsl_ipu_pool->allocator);
}


static void gst_fsl_ipu_buffer_pool_class_init(GstFslIpuBufferPoolClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	GstBufferPoolClass *parent_class = GST_BUFFER_POOL_CLASS(klass);

	GST_DEBUG_CATEGORY_INIT(fsl_ipubufferpool_debug, "fslipubufferpool", 0, "Freescale IPU buffer pool");

	object_class->finalize     = GST_DEBUG_FUNCPTR(gst_fsl_ipu_buffer_pool_finalize);
	parent_class->get_options  = GST_DEBUG_FUNCPTR(gst_fsl_ipu_buffer_pool_get_options);
	parent_class->set_config   = GST_DEBUG_FUNCPTR(gst_fsl_ipu_buffer_pool_set_config);
	parent_class->alloc_buffer = GST_DEBUG_FUNCPTR(gst_fsl_ipu_buffer_pool_alloc_buffer);
}
...
GstBufferPool *gst_fsl_ipu_buffer_pool_new(int ipu_fd, gboolean read_only)
{
	...
	fsl_ipu_pool = g_object_new(gst_fsl_ipu_buffer_pool_get_type(), NULL);
	fsl_ipu_pool->allocator = gst_fsl_ipu_allocator_new(ipu_fd);
	fsl_ipu_pool->read_only = read_only;

	return GST_BUFFER_POOL_CAST(fsl_ipu_pool);
}
