...
static gboolean gst_imx_phys_meta_transform (GstBuffer * dest, GstMeta * meta,
	GstBuffer * buffer, GQuark type, gpointer data)
{
	...
	if (GST_META_TRANSFORM_IS_COPY(type)) {
		...
		if (gst_buffer_n_memory(dest) == gst_buffer_n_memory(buffer) &&
			gst_buffer_n_memory(dest) == 1) {
			GstMemory *mem1, *mem2;
		...
		if (!copy->region && do_copy) {
			...
			dmeta = (GstImxPhysMemMeta *)
				gst_buffer_add_meta(dest, gst_imx_phys_mem_meta_get_info(), NULL);
...
static void
gst_imx_phys_meta_free (GstMeta * meta, G_GNUC_UNUSED GstBuffer * buffer)
{
...
GstMetaInfo const * gst_imx_phys_mem_meta_get_info(void)
{
	...
	if (g_once_init_enter(&gst_imx_phys_mem_meta_info))
	{
		GstMetaInfo const *meta = gst_meta_register(
			...
			(GstMetaFreeFunction)(gst_imx_phys_meta_free),
			(GstMetaTransformFunction)(gst_imx_phys_meta_transform)
		);
