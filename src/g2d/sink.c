...
static void gst_imx_g2d_video_sink_set_property(GObject *object, guint prop_id, GValue const *value, GParamSpec *pspec);
static void gst_imx_g2d_video_sink_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);

static gboolean gst_imx_g2d_video_sink_start(GstImxBlitterVideoSink *blitter_video_sink);
static gboolean gst_imx_g2d_video_sink_stop(GstImxBlitterVideoSink *blitter_video_sink);
...
void gst_imx_g2d_video_sink_class_init(GstImxG2DVideoSinkClass *klass)
{
	...
	object_class->set_property = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_sink_set_property);
	object_class->get_property = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_sink_get_property);

	base_class->start = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_sink_start);
	base_class->stop  = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_sink_stop);
...
static void gst_imx_g2d_video_sink_set_property(GObject *object, guint prop_id, GValue const *value, GParamSpec *pspec)
{
	GstImxG2DVideoSink *g2d_video_sink = GST_IMX_G2D_VIDEO_SINK(object);
...
static gboolean gst_imx_g2d_video_sink_start(GstImxBlitterVideoSink *blitter_video_sink)
{
	GstImxG2DVideoSink *g2d_video_sink = GST_IMX_G2D_VIDEO_SINK(blitter_video_sink);
	...
	g2d_video_sink->blitter = blitter;

	return TRUE;
}


static gboolean gst_imx_g2d_video_sink_stop(G_GNUC_UNUSED GstImxBlitterVideoSink *blitter_video_sink)
{
	return TRUE;
}
