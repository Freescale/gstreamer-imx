...
static void gst_imx_g2d_video_transform_set_property(GObject *object, guint prop_id, GValue const *value, GParamSpec *pspec);
static void gst_imx_g2d_video_transform_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);

gboolean gst_imx_g2d_video_transform_start(GstImxBlitterVideoTransform *blitter_video_transform);
gboolean gst_imx_g2d_video_transform_stop(GstImxBlitterVideoTransform *blitter_video_transform);
...
gboolean gst_imx_g2d_video_transform_are_transforms_necessary(GstImxBlitterVideoTransform *blitter_video_transform, GstBuffer *input);
...
void gst_imx_g2d_video_transform_class_init(GstImxG2DVideoTransformClass *klass)
{
	...
	object_class->set_property = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_transform_set_property);
	object_class->get_property = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_transform_get_property);

	base_class->start = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_transform_start);
	base_class->stop  = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_transform_stop);

	base_class->are_video_infos_equal = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_transform_are_video_infos_equal);
	
	base_class->are_transforms_necessary = GST_DEBUG_FUNCPTR(gst_imx_g2d_video_transform_are_transforms_necessary);
	...
	g_object_class_install_property(
		...
		g_param_spec_enum(
			...
			gst_imx_g2d_blitter_rotation_mode_get_type(),
			GST_IMX_G2D_BLITTER_OUTPUT_ROTATION_DEFAULT,
...
static void gst_imx_g2d_video_transform_set_property(GObject *object, guint prop_id, GValue const *value, GParamSpec *pspec)
{
	GstImxG2DVideoTransform *g2d_video_transform = GST_IMX_G2D_VIDEO_TRANSFORM(object);
...
static void gst_imx_g2d_video_transform_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	...
	switch (prop_id)
	{
		case PROP_OUTPUT_ROTATION:
			GST_IMX_BLITTER_VIDEO_TRANSFORM_LOCK(g2d_video_transform);
			g_value_set_enum(value, g2d_video_transform->output_rotation);
			GST_IMX_BLITTER_VIDEO_TRANSFORM_UNLOCK(g2d_video_transform);
			break;
...
gboolean gst_imx_g2d_video_transform_start(GstImxBlitterVideoTransform *blitter_video_transform)
{
	GstImxG2DVideoTransform *g2d_video_transform = GST_IMX_G2D_VIDEO_TRANSFORM(blitter_video_transform);
	...
	g2d_video_transform->blitter = blitter;

	return TRUE;
}


gboolean gst_imx_g2d_video_transform_stop(G_GNUC_UNUSED GstImxBlitterVideoTransform *blitter_video_transform)
{
	return TRUE;
}
...
gboolean gst_imx_g2d_video_transform_are_transforms_necessary(GstImxBlitterVideoTransform *blitter_video_transform, G_GNUC_UNUSED GstBuffer *input)
{
	GstImxG2DVideoTransform *g2d_video_transform = GST_IMX_G2D_VIDEO_TRANSFORM(blitter_video_transform);

	return gst_imx_g2d_blitter_get_output_rotation(g2d_video_transform->blitter) == GST_IMX_G2D_BLITTER_ROTATION_NONE;
}
