...
static gpointer gst_imx_pxp_map_phys_mem(GstImxPhysMemAllocator *allocator, GstImxPhysMemory *memory, gssize size, GstMapFlags flags);
...
static gboolean gst_imx_pxp_alloc_phys_mem(GstImxPhysMemAllocator *allocator, GstImxPhysMemory *memory, gssize size)
{
	...
	mem_desc->mtype = MEMORY_TYPE_UNCACHED;
	ret = pxp_get_mem(mem_desc);
	...
	{
		memory->mapped_virt_addr = (void*)(mem_desc->virt_uaddr);
...
static gboolean gst_imx_pxp_free_phys_mem(GstImxPhysMemAllocator *allocator, GstImxPhysMemory *memory)
{
	...
	{
		GST_ERROR_OBJECT(allocator, "could not free %u bytes of physical memory, vaddr %p paddr %" GST_IMX_PHYS_ADDR_FORMAT ": %s", memory->mem.size, memory->mapped_virt_addr, memory->phys_addr, strerror(errno));
		return FALSE;
	}
}
...
static void gst_imx_pxp_cache_op(GstImxPhysMemAllocator *allocator, GstImxPhysMemory *memory, int cache_mode)
{
	...
	if (ioctl(fd, PXP_IOC_FLUSH_PHYMEM, &flush) == 0)
	{
...
static gpointer gst_imx_pxp_map_phys_mem(GstImxPhysMemAllocator *allocator, GstImxPhysMemory *memory, G_GNUC_UNUSED gssize size, GstMapFlags flags)
{
...
/*	if (flags & GST_MAP_READ)
		gst_imx_pxp_cache_op(allocator, memory, PXP_CACHE_INVALIDATE);*/
...
/*	if (memory->mapping_flags & GST_MAP_WRITE)
		gst_imx_pxp_cache_op(allocator, memory, PXP_CACHE_CLEAN);*/
}
